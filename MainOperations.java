...
public class MainOperations {
    ...
    static orderService orderService = (com.demo.Service.orderService) new orderServiceImpl();
        ...
        while (true) {
            ...
            switch (input) {
                ...
                case 3:
                    ...
                    sc.nextLine(); // consume newline
                    order existingOrder = orderService.getOrder(orderIdToUpdate);
                    if (existingOrder != null) {
                        order updatedOrder = orderInputs();
                        updatedOrder.setOrderId(orderIdToUpdate); // preserve the original ID
    ...
    // Payment Operations
    public static paymentt paymentInputs() {
        System.out.println("Enter Brand:");
        String brand = sc.nextLine();
        ...
        sc.nextLine(); // consume newline
        return new paymentt(brand, unitPrice, quantity);
    }
    ...
    public static void paymentOperations() {
        while (true) {
            ...
            switch (input) {
                case 1:
                    paymentt pay = paymentInputs();
                    paymentService.savePaymentt(pay);
                    System.out.println("Payment added successfully.");
                    break;
                case 2:
                    List<paymentt> payments = paymentService.getAllPaymentts();
                    payments.forEach(System.out::println);
                    break;
                case 3:
                    System.out.println("Enter Brand to update:");
                    String brandToUpdate = sc.nextLine();
                    paymentt existingPayment = paymentService.getPaymentt(brandToUpdate);
                    if (existingPayment != null) {
                        paymentt updatedPayment = paymentInputs();
                        updatedPayment.setBrand(brandToUpdate); // preserve the original brand
                        paymentService.updatePaymentt(updatedPayment);
                ...
                case 4:
                    ...
                    String brandToDelete = sc.nextLine();
                    paymentService.deletePaymentt(brandToDelete);
    ...
    public static admin adminInputs() {
        ...
        String customerName = sc.nextLine();
        customer cust = customerService.getCustomer(customerName);
        System.out.println("Enter Payment Brand:");
        String paymentBrand = sc.nextLine();
        paymentt pay = paymentService.getPaymentt(paymentBrand);
        System.out.println("Enter Feedback Form:");
        String feedback;
		return null;
    }
}
